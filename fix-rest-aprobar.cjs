...
f = f.replace(
  ...
  `const existeViveres = await db().query("SELECT id FROM viveres WHERE owner_id = $1", [s.usuario_id]);
      if (existeViveres.length > 0) {
        await db().query("UPDATE viveres SET nombre=$1, tipo=$2, direccion=$3, status=''aprobado'' WHERE owner_id=$4",
          [s.datos?.nombre_negocio ?? "", (s.datos?.tipo_negocio ?? "").toLowerCase() === "restaurante" ? "restaurante" : "tienda", s.datos?.direccion ?? "", s.usuario_id]);
      } else {
...
console.log(f.includes("existeViveres") ? "OK" : "TEXTO NO ENCONTRADO");
