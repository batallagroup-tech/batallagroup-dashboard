...
import YaVoyAdmin from './pages/YaVoyAdmin';
  ...
  return (
    <button onClick={toggle} title={isFS ? 'Salir pantalla completa' : 'Pantalla completa'}
      style={{
        ...
        backdropFilter: 'blur(8px)', color: isFS ? '#3b82f6' : theme.textDim,
...
function ThemeButton({ dark, onToggle, theme }: { dark: boolean; onToggle: () => void; theme: Theme }) {
  return (
    <button onClick={onToggle} title={dark ? 'Modo claro' : 'Modo oscuro'}
      style={{
        position: 'fixed', bottom: 20, right: 68, zIndex: 9999,
        width: 38, height: 38, borderRadius: 10,
        background: theme.bg2, border: `1px solid ${theme.border}`,
...
function SearchOverlay({ onClose, onNavigate, theme }: {
  onClose: () => void;
  ...
  const STATIC: SearchResult[] = [
    ...
    { type: 'App', title: 'Ya Voy!', sub: 'App de reparto', screen: 'yavoy' },
    { type: 'Admin', title: 'Ya Voy! Admin', sub: 'Restaurantes, menús, repartidores', screen: 'yavoy-admin' },
  ...
  const TYPE_COLORS: Record<string, string> = {
    App: '#3b82f6', Admin: '#e91e8c', Stats: '#22c55e', Docs: '#f59e0b',
  };
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 18px', borderBottom: `1px solid ${theme.border}` }}>
          ...
          <input
            ...
            placeholder="Buscar pantalla, app, función…"
            style={{
        ...
        <div style={{ maxHeight: 340, overflowY: 'auto' as const }}>
          {results.map((r, i) => (
            <div key={i}
              onClick={() => { if (r.screen) { onNavigate(r.screen); onClose(); } }}
              style={{
        ...
        <div style={{ padding: '10px 18px', borderTop: `1px solid ${theme.border}` }}>
          <span style={{ color: theme.textDim, fontSize: 10, letterSpacing: '0.2em' }}>
...
const NOTIF_COLORS: Record<string, string> = {
  sos: '#ef4444', report: '#f59e0b', deploy: '#22c55e', info: '#3b82f6',
};
        ...
        <>
          ...
          <div style={{
            ...
            {notifs.map(n => (
              <div key={n.id} onClick={() => setNotifs(prev => prev.map(x => x.id === n.id ? { ...x, read: true } : x))}
                style={{
                  ...
                  borderLeft: n.read ? 'none' : `3px solid ${NOTIF_COLORS[n.type]}`,
                }}>
            ...
            <div style={{ padding: '10px 16px', textAlign: 'center' as const }}>
              <span style={{ color: theme.textDim, fontSize: 10, letterSpacing: '0.2em' }}>
                Las alertas reales se conectan con Supabase
              </span>
            </div>
          </div>
        </>
...
export default function App() {
  ...
  return (
    <>
      ...
      {screen === 'yavoy-admin' && <YaVoyAdmin onBack={() => setScreen('home')} theme={theme} />}
